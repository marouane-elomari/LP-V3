...
/* ─────────────────────────────────────────
   DOC TEMPLATES — driven by translations
───────────────────────────────────────── */
const getDocTemplates = (t: (key: string) => string) => [
  {
    id: 1,
    get name() { return t('fraud.doc1.name'); },
    get meta() { return t('fraud.doc1.meta'); },
    icon: FileText,
    color: "bg-orange-50 text-orange-600",
    outcomes: [
      { status: "verified", confidence: 97, message: t('fraud.doc1.out0') },
      { status: "verified", confidence: 94, message: t('fraud.doc1.out1') },
      { status: "verified", confidence: 91, message: t('fraud.doc1.out2') },
      { status: "warning",  confidence: 71, message: t('fraud.doc1.out3') },
      { status: "warning",  confidence: 63, message: t('fraud.doc1.out4') },
      { status: "rejected", confidence: 22, message: t('fraud.doc1.out5') },
    ],
  },
  {
    id: 2,
    get name() { return t('fraud.doc2.name'); },
    get meta() { return t('fraud.doc2.meta'); },
    icon: FileImage,
    color: "bg-blue-50 text-blue-600",
    outcomes: [
      { status: "verified", confidence: 99, message: t('fraud.doc2.out0') },
      { status: "verified", confidence: 93, message: t('fraud.doc2.out1') },
      { status: "warning",  confidence: 74, message: t('fraud.doc2.out2') },
      { status: "warning",  confidence: 61, message: t('fraud.doc2.out3') },
      { status: "warning",  confidence: 58, message: t('fraud.doc2.out4') },
      { status: "rejected", confidence: 18, message: t('fraud.doc2.out5') },
    ],
  },
  {
    id: 3,
    get name() { return t('fraud.doc3.name'); },
    get meta() { return t('fraud.doc3.meta'); },
    icon: Briefcase,
    color: "bg-purple-50 text-purple-600",
    outcomes: [
      { status: "verified", confidence: 98, message: t('fraud.doc3.out0') },
      { status: "verified", confidence: 95, message: t('fraud.doc3.out1') },
      { status: "verified", confidence: 88, message: t('fraud.doc3.out2') },
      { status: "warning",  confidence: 69, message: t('fraud.doc3.out3') },
      { status: "warning",  confidence: 55, message: t('fraud.doc3.out4') },
      { status: "rejected", confidence: 11, message: t('fraud.doc3.out5') },
    ],
  },
];
...
function generateRun(t: (key: string) => string) {
  const DOC_TEMPLATES = getDocTemplates(t);
  ...
  if      (trustScore >= 88) trustLabel = t('fraud.trustLabel.excellent');
  else if (trustScore >= 72) trustLabel = t('fraud.trustLabel.good');
  else if (trustScore >= 55) trustLabel = t('fraud.trustLabel.moderate');
  else if (trustScore >= 35) trustLabel = t('fraud.trustLabel.poor');
  else                        trustLabel = t('fraud.trustLabel.highRisk');
  ...
  const parts: string[] = [];
  if (v) parts.push(`${v} ${t('fraud.verified').toLowerCase()}`);
  if (w) parts.push(`${w} ${t('fraud.flagged').toLowerCase()}`);
  if (r) parts.push(`${r} ${t('fraud.reject').toLowerCase()}`);
...
/* ─────────────────────────────────────────
   ANIMATION HELPERS
───────────────────────────────────────── */
function AnimatedBar({ width, delay = 0, resetKey }: { width: number; delay?: number; resetKey: number }) {
  const [w, setW] = useState(0);
  useEffect(() => {
    setW(0);
    const timer = setTimeout(() => setW(width), delay + 20);
...
export default function FraudDetection() {
  ...
  const [scenario,   setScenario]   = useState<ReturnType<typeof generateRun> | null>(null);
  const [hovered,    setHovered]    = useState(false);
  const timerRef = useRef<ReturnType<typeof setInterval> | null>(null);

  // Build default doc list (for idle state)
  const DOC_TEMPLATES = getDocTemplates(t);
  ...
  const documents = scenario ? scenario.documents : DOC_TEMPLATES.map(tmpl => ({ ...tmpl, ...tmpl.outcomes[0] }));
  ...
  const btnDisabled = status === "running";
  const btnBg = btnDisabled
    ? '#9ca3af'
    : hovered
      ? 'linear-gradient(135deg, #ff7f5a 0%, #c96a50 100%)'
      : 'linear-gradient(135deg, #FE9170 0%, #e07a5f 100%)';
    ...
    <section id="fraud-detection" className="py-12 md:py-20 px-4 md:px-8 font-sans min-h-screen">
      ...
      <div className="max-w-3xl mx-auto text-center mb-12 md:mb-16">
        <div
          ...
          style={{ backgroundColor: "rgba(254,145,112,0.1)", color: "#FE9170", borderColor: "rgba(254,145,112,0.2)" }}
        >
          <Sparkles className="w-4 h-4" />
          <span className="text-sm font-medium uppercase tracking-wide">{t('fraud.badge')}</span>
        </div>
        <h2
          className="text-3xl md:text-4xl lg:text-5xl text-gray-900 mb-4 md:mb-6 leading-tight font-bold px-4"
          style={{ fontFamily: 'Instrument Serif, serif' }}
        >
          {t('fraud.title1')}{" "}
          <span className="italic" style={{ color: "#FE9170", fontFamily: 'Instrument Serif, serif' }}>
            {t('fraud.title2')}
          </span>
        </h2>
        <p className="text-base md:text-lg text-gray-600 leading-relaxed px-4">
          {t('fraud.subtitle')}
          <br />
          <span className="text-sm text-gray-400 mt-1 block">{t('fraud.randomNote')}</span>
        </p>
      </div>
      ...
      <div className="max-w-5xl mx-auto space-y-6 md:space-y-8">
        ...
        <div className="bg-white rounded-xl shadow-sm border border-gray-200 overflow-hidden">
          ...
          <div className="bg-gradient-to-r from-gray-50 to-white border-b border-gray-200 px-6 py-3 flex items-center justify-between">
            <div className="flex items-center gap-6">
              <div className="flex gap-1.5">
                <div className="w-2.5 h-2.5 rounded-full" style={{ backgroundColor: '#FE9170' }} />
          ...
          <div className="p-8 space-y-12 bg-gradient-to-br from-white via-gray-50/30 to-purple-50/20">
            ...
            <div className="flex flex-col md:flex-row md:items-end justify-between gap-6">
              ...
              {/* ── RUN ANALYSIS BUTTON with hover ── */}
              <button
                onClick={handleRunAnalysis}
                disabled={btnDisabled}
                onMouseEnter={() => setHovered(true)}
                onMouseLeave={() => setHovered(false)}
                className="px-6 py-2.5 rounded-full text-white text-[10px] uppercase tracking-widest
                           disabled:opacity-50 disabled:cursor-not-allowed cursor-pointer
                           transition-all duration-200"
                style={{
                  fontFamily: 'DM Mono, monospace',
                  background: btnBg,
                  boxShadow: hovered && !btnDisabled
                    ? '0 8px 24px rgba(254,145,112,0.40)'
                    : '0 2px 8px rgba(254,145,112,0.20)',
                  transform: hovered && !btnDisabled ? 'scale(1.04)' : 'scale(1)',
                }}
              >
                {status === "running" ? t("fraud.analyzing") : t("fraud.runButton")}
              </button>
            </div>
            ...
            <div className="grid grid-cols-1 lg:grid-cols-3 gap-8">
              ...
              <div className="lg:col-span-2 space-y-8">
                ...
                <div className="bg-white rounded-xl border border-gray-200 p-6 shadow-sm">
                  ...
                  <div
                    className="flex justify-between text-[8px] uppercase tracking-widest text-gray-400"
                ...
                <div className="bg-white rounded-xl border border-gray-200 p-6 shadow-sm">
                  <div
                    className="text-[9px] uppercase tracking-widest mb-6 text-gray-400"
                    style={{ fontFamily: 'DM Mono, monospace' }}
                  >
                    ...
                    {documents.map((doc, idx) => {
                      ...
                      return (
                        <div
                          ...
                          style={{
                            borderColor: docStatus === "scanning" ? '#FE9170' : '#f3f4f6',
                            backgroundColor: docStatus === "scanning" ? 'rgba(254,145,112,0.03)' : 'white',
                          }}
                        >
                          <div className="flex items-center gap-4">
                            <div
                              className={`w-10 h-10 rounded-lg border border-gray-200 flex items-center justify-center bg-gradient-to-br ${
                            ...
                            <div>
                              ...
                              <p
                                className="text-[10px] text-gray-400 uppercase tracking-wide mt-0.5"
                                style={{ fontFamily: 'DM Mono, monospace' }}
                              >
                                {doc.meta}
                              </p>
                          ...
                          <div className="min-w-[60px] flex justify-end">
                            {docStatus === "completed" ? (
                              <FadeIn show={true} delay={idx * 80} resetKey={runKey}>
                                ...
                                >
                                  {doc.status === "verified" ? t('fraud.ok') : doc.status === "warning" ? t('fraud.flag') : t('fraud.reject')}
                                </span>
                              </FadeIn>
                            ) : docStatus === "scanning" ? (
                              <span
                                className="text-[9px] uppercase tracking-widest font-semibold"
                                style={{ fontFamily: 'DM Mono, monospace', color: '#FE9170' }}
                              >
                                {t('fraud.scan')}
                              </span>
                            ) : (
                              <span
                                className="text-[9px] uppercase tracking-widest text-gray-300"
                                style={{ fontFamily: 'DM Mono, monospace' }}
                              >
                                —
                              </span>
                ...
                <div className="flex items-center justify-between mb-8">
                  ...
                  {status === "completed" && (
                    <FadeIn show={true} resetKey={runKey}>
                      <span
                        className="text-[9px] uppercase tracking-widest font-semibold"
                        style={{ fontFamily: 'DM Mono, monospace', color: '#000000' }}
                      >
                        {t('fraud.done')}
                      </span>
                    </FadeIn>
                ...
                <div className="space-y-6 pt-4">
                  ...
                  <div className="flex items-baseline justify-between">
                    ...
                    <div className="flex items-baseline gap-3">
                      ...
                      <span
                        className="text-3xl font-bold bg-gradient-to-r from-orange-500 to-purple-500 bg-clip-text text-transparent"
                        style={{ fontFamily: 'Instrument Serif, serif', fontStyle: 'italic' }}
                      >
                        {scenario.trustScore}
                      </span>
                  ...
                  <div className="h-2 bg-gray-100 rounded-full overflow-hidden">
                    <div
                      ...
                      style={{
                        ...
                        background: 'linear-gradient(90deg, #FE9170 0%, #B8A8FE 100%)',
                      }}
                    />
                  </div>
                      ...
                      return (
                        <FadeIn key={`${doc.id}-${runKey}`} show={true} delay={300 + idx * 100} resetKey={runKey}>
                          <div className="bg-white rounded-xl border border-gray-200 p-5 shadow-sm hover:shadow-md transition-shadow">
                            <div
                              className="text-[9px] uppercase tracking-widest mb-3 text-gray-400"
                              style={{ fontFamily: 'DM Mono, monospace' }}
                            >
                              {doc.id === 1
                                ? t('fraud.doc.income')
                                : doc.id === 2
                                  ? t('fraud.doc.identity')
                                  : t('fraud.doc.employment')}
                            </div>
                            ...
                            <div className="space-y-3">
                              ...
                              <p
                                className="text-[10px] text-gray-600 leading-relaxed pt-2 border-t border-gray-100"
        ...
        <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
          <div className="bg-white rounded-xl border border-gray-200 p-6 shadow-sm">
            <div
              className="text-[9px] uppercase tracking-widest mb-2 text-gray-400"
              style={{ fontFamily: 'DM Mono, monospace' }}
            >
